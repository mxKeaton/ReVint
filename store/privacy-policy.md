...
ReVint only acts on pages you open yourself. It does not scan other websites and does not collect data about other users.

## What ReVint stores and where

All saved listings — including the photos as text (Base64) — are stored **locally on your device**, inside the extension's own storage (IndexedDB). Nothing is uploaded anywhere. There is no account, no server and no cloud sync.

## What ReVint shares

Nothing. ReVint has no network endpoints of its own and transmits no data to the developer or any third party. The only network requests it makes are to the Vinted pages and image URLs described above, on your behalf.
